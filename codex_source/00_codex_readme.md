...
已确认：本项目是 `视频能力实验室｜Codex 视频导演能力库`。  
已确认：本项目用于验证、沉淀和复用 Codex 视频执行能力。  
已确认：本项目当前不是正式出片项目。

## 当前任务类型

当前任务类型为 `mechanism_sync` / `scaffold_initialization`，不是视频生成、Remotion、HyperFrames 或 BGM 真实验证。

## 强制 push 规则

每轮 Codex 任务只要产生仓库文件改动，就必须 commit + push。只有远端 HEAD 验证成功后，最终状态才能写 `completed`。

## 状态标记规则

必须使用 `已确认` / `部分成立` / `待验证` / `推测` / `通用建议`。未验证能力一律写 `待验证`。

## 禁止旧项目事实污染

源项目只提供协作机制参考；不得迁移旧项目业务目标、素材路径、模型选择、完成状态或验收结果。
