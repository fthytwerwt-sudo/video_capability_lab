...
- 已确认：本轮目标是补齐“外部图像资产生成判断与视频匹配流程”，不是正片，不是发布候选，不是 `video_fixed`。
- 已确认：本轮必须写入 `this_is_partial_pipeline_alignment_candidate_not_full_video_candidate`。
- 已确认：本轮保留 `55_参考视觉语言迁移库_reference_visual_language_migration_library.md`、`56_字幕贴纸视觉语言判断路由器_caption_sticker_visual_language_decision_router.md`、`59_字幕贴纸视觉回审闭环_caption_sticker_visual_review_loop.md`、`61_视觉前处理数据协议_visual_preprocessing_data_protocol.md` 原机制不变。
- 已确认：本轮没有修改 BGM、`beat_map`、音乐卡点、基础调色或 `color_grade_profile`。
- 已确认：本轮没有调用阿里 API，没有生成新图，只复用当前已有 6 个 alpha 资产。
- 已确认：新增流程文件为 `项目资料_docs/视频能力实验室_video_capability_lab/73_外部图像资产生成判断与视频匹配流程_external_image_asset_generation_video_fit_pipeline.md`。
- 已确认：新增候选报告为 `项目资料_docs/视频能力实验室_video_capability_lab/74_外部图像资产流程对齐候选报告_external_image_asset_pipeline_alignment_candidate_report.md`。
- 已确认：新增 Remotion data 为 `remotion/数据_data/外部图像资产流程对齐候选_external_image_asset_pipeline_alignment_candidate.ts`。
- 已确认：新增 Remotion composition 为 `remotion/组合_compositions/外部图像资产流程对齐候选_external_image_asset_pipeline_alignment_candidate.tsx`，并在 `remotion/Root.tsx` 注册 composition id `外部图像资产流程对齐候选-external-image-asset-pipeline-alignment-candidate`。
- 已确认：新增机器自检脚本为 `脚本_scripts/外部图像资产流程_external_image_asset_pipeline/生成外部图像资产匹配自检_generate_external_image_asset_fit_check.py`。
- 已确认：新增审片包脚本为 `脚本_scripts/外部图像资产流程_external_image_asset_pipeline/生成外部图像资产流程审片包_generate_external_image_asset_pipeline_review_pack.py`。
- 已确认：Remotion 流程对齐候选已渲染到 `dist/外部图像资产流程对齐候选_external_image_asset_pipeline_alignment_candidate/外部图像资产流程对齐候选_external_image_asset_pipeline_alignment_candidate.mp4`。
- 已确认：输出视频技术验证通过：`8.042667s format / 8.000000s video stream / 1080x1920 / 30fps / h264 / AAC stereo / decodable=true`。
- 已确认：审片包已生成到 `tmp/外部图像资产流程对齐候选_external_image_asset_pipeline_alignment_candidate_review_pack/`，包含 before / after、asset、alpha、video fit contact sheet、`asset_generation_spec.json`、`asset_video_fit_check.json`、`review_manifest.json` 和 readable report。
...
- 部分成立：外部图像资产链路已从视频事件锚点、视觉语言判断、资产规格、alpha、video fit check 接回 Remotion 8 秒流程对齐候选。
- 待验证：用户 / GPT 仍需审看 01-04 的图像内容、形状、情绪、浮层感、遮挡关系和视频事件匹配。
- 特别记录：`asset_need_05` 原需求为 `顺线`，实际补位文字为 `没感觉`；`asset_need_06` 原需求为 `线走这边`，实际补位文字为 `没感觉`。二者均为 `provider_arrearage_fallback_existing_ali_asset / text_semantic_mismatch_due_to_fallback_asset / chain_test_only_regenerate_required`，不得计入原需求生成成功。
- 禁止声明：`full_video_candidate`、`approved_for_video`、`asset_library_completed`、`caption_font_system_completed`、`video_fixed`、`publish-ready`、`vlog_director_capability_verified`。
...
- 已确认：本轮目标是补齐 Remotion plugin layer、visual preprocessing protocol layer 和 real visual tool adapter layer；不是生成完整正片，不是重剪当前视频，不是证明视频导演能力已成立。
- 已确认：本轮以项目 `package.json`、`package-lock.json`、`node_modules`、`npm ls` 和 import smoke test 为依赖事实源；Codex 插件环境不等于本项目已安装依赖。
- 已确认：当前 Remotion 版本为 `4.0.469`，本轮新增并锁定 `@remotion/paths@4.0.469`、`@remotion/motion-blur@4.0.469`、`@remotion/effects@4.0.469`。
- 已确认：新增协议文件 `项目资料_docs/视频能力实验室_video_capability_lab/61_视觉前处理数据协议_visual_preprocessing_data_protocol.md`，定义 `anchor_map.json`、`motion_track.json`、`mask_plan.json` 和 `visual_scorecard.json` 的用途、字段和 JSON schema。
- 已确认：新增工具链补强报告 `项目资料_docs/视频能力实验室_video_capability_lab/62_Remotion插件与视觉工具链补强报告_remotion_plugin_and_visual_toolchain_upgrade_report.md`。
- 已确认：新增 Remotion 插件冒烟探针 data / composition，并在 `remotion/Root.tsx` 注册 `视觉工具链插件冒烟探针-visual-toolchain-plugin-smoke-probe`。
- 已确认：新增视觉前处理驱动字幕贴纸探针 data / composition，并在 `remotion/Root.tsx` 注册 `视觉前处理驱动字幕贴纸探针-visual-preprocessing-driven-caption-sticker-probe`。
- 已确认：新增脚本目录 `脚本_scripts/视觉前处理_visual_preprocessing/`，包含 OpenCV anchor / motion / mask / scorecard、MediaPipe landmarks、SAM2 adapter/env probe、环境检查和审片包脚本。
- 已确认：SAM2 本轮只做 adapter 和环境探测，不下载 checkpoints，不 clone 外部仓库，不安装大型 CUDA / torch 依赖；无权重时必须写 `interface_ready_weights_missing`。
- 已确认：两个 Remotion 2-4 秒 probe 已 render 成功；第一次 effects render 遇到 WebGL2 context 问题后，按 Remotion 错误提示用 `--gl=angle` 复跑通过。
- 已确认：两个 probe 均通过 `ffprobe` 元数据检查和 `ffmpeg` decode check，均为 `h264 / 1080x1920 / 30fps / stream_duration=3.000000 / format_duration=3.050667`。
- 已确认：OpenCV probe 已输出 `anchor_map.json`、`motion_track.json`、`mask_plan.json`、`visual_scorecard.json`、edge preview、motion mask preview 和 `opencv_probe_report.json` 到 ignored `tmp/视觉前处理探针_visual_preprocessing_probe/`。
- 部分成立：MediaPipe 已安装并可导入，当前 `mediapipe 0.10.35` 暴露 `tasks` API，不含旧 `solutions`，本轮无本地 `.task/.tflite` 模型；probe 输出 `no_landmark_detected.json`，未伪造关键点。
- 已确认：SAM2 adapter 和环境探测已建立；当前无 torch / torchvision，无本地权重，状态为 `interface_ready_weights_missing`，未下载模型权重。
- 已确认：审片包已生成到 ignored `tmp/视觉工具链补强审片包_visual_toolchain_upgrade_review_pack/`，包含两条 probe 关键帧、contact sheet 和 runtime JSON manifest。
...
- 待验证：用户仍需审看本轮工具链补强审片包；commit、push 和 remote HEAD readback 由最终 Codex 回报给出。
...
- 已确认：本轮任务为 `caption_sticker_2_4s_fix_v2_micro_probe`。
- 已确认：本轮基于 `58_字幕贴纸2到4秒微段精修探针报告_caption_sticker_2_4s_micro_probe_report.md` 继续修正 5 个“总差一点”问题，不重新生成 18 秒完整正片，不增加贴纸数量，不声明能力证明。
- 已确认：本轮新增机制文件 `项目资料_docs/视频能力实验室_video_capability_lab/59_字幕贴纸视觉回审闭环_caption_sticker_visual_review_loop.md`，写入 `frame_review_loop`、`visual_scorecard`、`caption_sticker_fix_spec` 和 `micro_probe_before_full_render_2_4s`。
- 已确认：后续字幕 / 贴纸 / 视觉标点如果出现“像口号、像组件、锚点不贴画面、材质浮、动效像参数动画”，必须先走 `59`；字幕贴纸没过 2-4 秒微段回审前，不允许直接扩到整条 18 秒正片。
- 已确认：本轮继续读取并使用 `54_解析资产全量索引_analysis_asset_inventory.md`、`55_参考视觉语言迁移库_reference_visual_language_migration_library.md`、`56_字幕贴纸视觉语言判断路由器_caption_sticker_visual_language_decision_router.md`，并额外使用 `59`。
- 已确认：v2 继续选取当前候选 `3.15-6.15s` 窗口，来源为 `seg_04_main_action_push -> seg_05_metal_texture_cut`，素材为 `M03 + M06`，BGM 仍沿用 `素材/剪辑素材/BGM/copy_C23D419B-74B9-48BB-A971-F8D19ADE885F.MOV`。
- 已确认：v2 修正把 v1 大字“顶一下”降权为接触窗口的小型拟声 `咔`；把通用感更强的 pressure burst / tail / residual 改为贴机器圆孔、横杆边缘和 M06 金属纹理的 rub mark / scuff / absorb smear。
- 已确认：v2 每个字幕 / 贴纸 / 视觉标点事件均写入 `analysis_asset_ids`、`reference_rule_links`、`caption_visual_language_decision`、`sticker_visual_language_decision`、`caption_sticker_relation`、`copy_risk_check` 和 `template_fallback=false`。
- 已确认：新增 Remotion data：`remotion/数据_data/字幕贴纸2到4秒微段修正版_caption_sticker_2_4s_fix_v2.ts`。
- 已确认：新增 Remotion composition：`remotion/组合_compositions/字幕贴纸2到4秒微段修正版_caption_sticker_2_4s_fix_v2.tsx`，并在 `remotion/Root.tsx` 注册 composition id `字幕贴纸2到4秒微段修正版-caption-sticker-2-4s-fix-v2`。
- 已确认：本地输出视频路径为 `dist/字幕贴纸2到4秒微段修正版_caption_sticker_2_4s_fix_v2/字幕贴纸2到4秒微段修正版_caption_sticker_2_4s_fix_v2.mp4`。
- 已确认：输出视频技术验证通过：`3.050667s / 1080x1920 / 30fps / h264 / AAC stereo / decodable=true`。
- 已确认：审片包已生成到 `tmp/字幕贴纸2到4秒微段修正版_caption_sticker_2_4s_fix_v2_review_pack/`，包含原 18 秒候选对应帧、v1 微段帧、v2 修正版帧、before/v1/v2 contact sheet、`visual_scorecard.json` 和 `fix_spec.json`。
- 已确认：新增报告为 `项目资料_docs/视频能力实验室_video_capability_lab/60_字幕贴纸2到4秒微段修正版报告_caption_sticker_2_4s_fix_v2_report.md`。
...
- 待验证：用户仍需审看本地 v2 微段，判断 `咔`、pin rub、bar scuff 和 texture absorb 是否真正比 v1 更像动作事件带出的帧内反应；不得声明 `publish-ready`、`video_fixed`、`full video candidate completed`、`vlog director capability verified` 或用户审美通过。
